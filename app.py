...
def usuario_pode_editar_pedido(pedido):
    ...
    if perfil in ["diretoria", "supervisor"]:
...
def carregar_pedidos():
    ...
    mapa_colunas = {
        ...
        "comprador": "Comprador",
        "obra": "Obra",
        "material_e_quantidade": "Material e quantidade",
        "data_limite_para_compra": "Data Limite Para Compra",
        "data_limite_para_entrega": "Data Limite Para Entrega",
        "prazo_entrega_fornecedor": "Prazo de Entrega do Fornecedor",
    ...
    if "ID Pedido" in df.columns:
        df = df[df["ID Pedido"].astype(str).str.strip().ne("")]
...
def salvar_pedido_supabase(id_pedido, dados_editados):
    supabase = conectar_supabase()

    mapa_colunas = {
        "Obra": "obra",
        "Material e quantidade": "material_e_quantidade",
        "Data Limite Para Entrega": "data_limite_para_entrega",
        "Prazo de Entrega do Fornecedor": "prazo_entrega_fornecedor",
    ...
    for coluna_app, valor in dados_editados.items():
        coluna_banco = mapa_colunas.get(coluna_app)

        if coluna_banco:
            dados_update[coluna_banco] = valor

    if not dados_update:
        return False, "Nenhuma informação válida para atualizar."
...
def abrir_modal_edicao(pedido):
    ...
    with col_salvar:
        if st.button("Salvar alterações", type="primary", use_container_width=True):
            ...
            sucesso, mensagem = salvar_pedido_supabase(id_pedido, dados_editados)
...
def tela_pedidos(df):
    ...
    mostrar_cards_pedidos(
        df_filtrado,
        permitir_edicao=(perfil in ["comprador", "diretoria", "supervisor"])
    )
...
def app_principal():
    ...
    if perfil in ["comprador", "diretoria", "supervisor", "admin"]:
        tela_pedidos(df)
    else:
